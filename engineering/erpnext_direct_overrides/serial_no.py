# # Path = erpnext/stock/doctype/serial_no/serial_no.py
	...
	def get_stock_ledger_entries(self, serial_no=None):
		...
		incoming_sle = frappe.db.sql("""
			SELECT voucher_type, voucher_no,
				posting_date, posting_time, incoming_rate, actual_qty
			FROM
				`tabStock Ledger Entry`
			FORCE INDEX (company_w_item_posting_creation_index)
			WHERE
				company = %s AND warehouse=%s AND item_code=%s
				AND (serial_no = %s
					OR serial_no like %s
					OR serial_no like %s
					OR serial_no like %s
			...
			limit 2""",
			(self.company, self.old_warehouse, self.item_code,
				serial_no, serial_no+'\n%', '%\n'+serial_no, '%\n'+serial_no+'\n%'), as_dict=1)

		if incoming_sle:
			for idx,sle in enumerate(incoming_sle):
				if sle.actual_qty > 0:
					sle_dict.setdefault("incoming", []).append(sle)
				if sle.actual_qty < 0 and idx == 1:
					sle_dict.setdefault("outgoing", []).append(sle)
		# outgoing_sle = frappe.db.sql("""
		# 	SELECT voucher_type, voucher_no,
		# 		posting_date, posting_time, incoming_rate, actual_qty
		# 	FROM
		# 		`tabStock Ledger Entry`
		# 	FORCE INDEX (company_w_item_posting_creation_index)
		# 	WHERE
		# 		company = %s AND warehouse=%s AND item_code=%s AND actual_qty < 0
		# 		AND (serial_no = %s
		# 			OR serial_no like %s
		# 			OR serial_no like %s
		# 			OR serial_no like %s
		...
		# if outgoing_sle:
		# 	sle_dict.setdefault("outgoing", []).append(outgoing_sle[0])
		...
		# 	WHERE
		# 		company = %s AND warehouse=%s AND item_code=%s AND ifnull(is_cancelled, 'No') = 'No'
		# 		AND (serial_no = %s
		# 			OR serial_no like %s
		# 			OR serial_no like %s
		# 			OR serial_no like %s
			...
			for serial_no in serial_nos:
				if frappe.db.exists("Serial No", serial_no):
					...
					if cint(sle.actual_qty) > 0 and has_duplicate_serial_no(sr, sle):
						if sle.voucher_type not in ['Stock Entry', 'Stock Reconciliation'] and sr.delivery_document_no:
...
def update_args_for_serial_no(serial_no_doc, serial_no, args, is_new=False):
	serial_no_doc.old_warehouse = (args.get("warehouse")
			if args.get("actual_qty", 0) > 0 else serial_no_doc.warehouse)
...
# Finbyz Changes OLD serial_no py file
...
# 			ORDER BY
# 				posting_date desc, posting_time desc, creation desc""",
# 			(self.company,self.warehouse, self.item_code,
# 				serial_no, serial_no+'\n%', '%\n'+serial_no, '%\n'+serial_no+'\n%'), as_dict=1):
# 				if serial_no.upper() in get_serial_nos(sle.serial_no):
# 					if cint(sle.actual_qty) > 0:
# 						sle_dict.setdefault("incoming", []).append(sle)
