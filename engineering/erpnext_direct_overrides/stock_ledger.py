...
class update_entries_after(object):
	...
	def __init__(self, args, allow_zero_rate=False, allow_negative_stock=None, via_landed_cost_voucher=False, verbose=1):
		...
		# Finbyz Changes: change_rate of purchase_receipt from purchase_invoice
		self.change_rate = args.get('change_rate') or False
		# Finbyz Changes End
	...
	def build(self):
		...
		for sle in entries_to_fix:
			...
			if not sle.serial_no or self.change_rate:
				if frappe.db.get_value("Item",sle.item_code,"has_serial_no") and not sle.serial_no:
					self.process_sle_after(sle)
				else:
	...
	# Finbyz Changes : Copy of process_sle_after(self, sle) Function: Not Evaluate Valuation Rate Again for Serial No
	def process_sle_after(self, sle):
		if (sle.serial_no and not self.via_landed_cost_voucher) or not cint(self.allow_negative_stock):
			# validate negative stock for serialized items, fifo valuation
			# or when negative stock is not allowed for moving average
			if not self.validate_negative_stock(sle):
		...
		if sle.serial_no or (not sle.serial_no and frappe.db.get_value("Item",sle.item_code,"has_serial_no")):
			# Finbyz Changes START
			self.get_serialized_values_after(sle)
			# Finbyz Changes END
			self.qty_after_transaction += flt(sle.actual_qty)
			if sle.voucher_type == "Stock Reconciliation":
				self.qty_after_transaction = sle.qty_after_transaction
	...
	# Finbyz Changes START: Copy of get_serialized_values_after(self, sle) Function: Not Evaluate Valuation Rate Again for Serial No
	def get_serialized_values_after(self, sle):
		
		incoming_rate = flt(sle.incoming_rate)
		actual_qty = flt(sle.actual_qty)
		serial_nos = cstr(sle.serial_no).split("\n") if sle.serial_no else None
		...
		# Finbyz Changes Start
			if sle.stock_value_difference or sle.stock_value_difference != 0:
				outgoing_value = -1 * sle.stock_value_difference
			else:
	...
	def get_incoming_value_for_serial_nos(self, sle, serial_nos):
		if serial_nos:
			...
			# Finbyz Changes START : Using Index item_code_index
			...
			# Finbyz Changes Start: if Same Company but Purchase Rate is not found
			invalid_serial_nos += [d.name for d in all_serial_nos if d.company==sle.company and not d.purchase_rate]
			# Finbyz Changes End

			# Finbyz Changes START : Using INDEX company_warehouse_item_index and filter applied for warehouse and item_code
			...
			previous_entry_valuation = frappe.db.sql("""
				select valuation_rate
				from `tabStock Ledger Entry`
				where company = '{}' and warehouse = '{}' and item_code = '{}' and (valuation_rate is not null and valuation_rate != 0)
				order by posting_date desc, posting_time desc, creation desc
				limit 1
			""".format(sle.company,sle.warehouse,sle.item_code))
		
			if previous_entry_valuation:
				incoming_values = flt(previous_entry_valuation[0][0]) if previous_entry_valuation else 0
			return incoming_values
...
# Old Stock ledger py file before changes of get outgoing rate of serialized item without serial nos
...
# Finbyz Changes: old stock_ledger py file
