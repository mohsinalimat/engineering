...
def get_incoming_rate(args, raise_error_if_no_rate=True):
	...
	if (args.get("serial_no") or "").strip():
		...
		#finbyz changes start : for using INDEX (active_serial_index)
		in_rate = get_avg_purchase_rate(args.get("serial_no"),args.get("company"),args.get("warehouse"),args.get("item_code"))
		#finbyz changes END
	else:
...
def get_avg_purchase_rate(serial_nos,company,warehouse,item_code):
	...
	#Finbyz Changes START : for using INDEX (active_serial_index)
	return flt(frappe.db.sql(f"""select avg(purchase_rate) from `tabSerial No`
		FORCE INDEX (active_serial_index)
		where status = 'Active' and company = '{company}' and warehouse = '{warehouse}' and item_code = '{item_code}' and name in (%s)""" % ", ".join(["%s"] * len(serial_nos)),
		tuple(serial_nos))[0][0])
	#Finbyz Changes END
