...
def prepare_data(accounts, balance_must_be, period_list, company_currency):
	...
	for d in accounts:
		...
		has_value = False
		total = 0
		# Finbyz Changes START: "show_report":d.show_report
		row = frappe._dict({
			"account": _(d.name),
...
def get_accounts(company, root_type):
	# Finbyz Changes START:  selected show_report in query
	return frappe.db.sql("""
		select name, account_number, parent_account, lft, rgt, root_type, report_type, account_name, include_in_gross, account_type, show_report,is_group, lft, rgt
		from `tabAccount`
